...
from flexgen.ref_hf import RotaryEmbedding, apply_rotary_pos_emb, repeat_kv
...
class TorchDevice:
    ...
    def input_embed(self, inputs, w_token, pad_token_id, donate):
        # decompress weights
        ...
        # token embedding
        input_embed = F.embedding(token_ids, w_token.data, pad_token_id)
        
        return TorchTensor.create_from_torch(input_embed, self)

    def output_embed(self, inputs, w_norm, w_lm_head, donate,
                         do_sample, temperature):
        
        b, s, h = inputs.shape

        hidden = F.rms_norm(inputs.data, (h,), weight=w_norm.data, eps=1e-6)
        if donate[0]: inputs.delete()
    ...
    def mha(self, hidden, attention_mask, w_qkv,
            w_out, w_ln, n_head, n_kv_head, start_pos, nl, nb, cache_read_buf, device, cache_write_buf):
        """Multi-head attention (prefill phase)."""

        hidden = F.layer_norm(hidden.data, (h,), weight=w_ln.data)
        b, q_len, h = hidden.size()
        
        cache_position = torch.arange(
                start_pos, start_pos + q_len, device='cuda')
        ...
        qkv = F.linear(hidden, w_qkv.data)
        query_pos = n_head * head_dim

        xq = qkv[..., : query_pos]
        xk = qkv[..., : query_pos : query_pos + n_kv_head*head_dim]
        xv = qkv[..., : query_pos + n_kv_head*head_dim : ]

        # shape (b, s, n_head, head_dim)
        xq = xq.view(b, q_len, n_head, head_dim)
        xk = xk.view(b, q_len, n_kv_head, head_dim)
        xv = xv.view(b, q_len, n_kv_head, head_dim)
        ...
        cos, sin = rotary_emb(xv, position_ids, seq_len=kv_seq_len)
        xq, xk = apply_rotary_pos_emb(xq, xk, cos, sin)
        

        ##shape (b, s, n_head, head_dim)
        keys = torch.concat(cache_read_buf[nl][nb][0], xk, dim=1)
        values = torch.concat(cache_read_buf[nl][nb][1], xv, dim=1)
        
        if n_kv_head < n_head:
            keys = repeat_kv(keys, n_head//n_kv_head)
            values = repeat_kv(values, n_head//n_kv_head)
        ...
        attn_output = attn_output.reshape(b, q_len, h).contiguous()
        attn_output = w_out(attn_output)

        k = TorchTensor.create_from_torch(xk, self)
        v = TorchTensor.create_from_torch(xv, self)

        return TorchTensor.create_from_torch(attn_output, self), k, v, kv_seq_len
    ...
    def mlp(self, inputs, w_up, w_down):
        
        out = F.linear(inputs.data, w_up.data)
        gate, out = out.chunk(2, dim=-1)
        out = out * F.sliu(gate)
        out = F.linear(out, w_down)
