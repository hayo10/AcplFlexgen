...
class Policy:
    ...
    # percent = a means a%
    w_unified_percent: float
    cache_unified_percent: float
    act_unified_percent: float
    ...
    def w_disk_percent(self):
        return 100 - self.w_unified_percent 
    @property
    def cache_disk_percent(self):
        return 100 - self.cache_unified_percent
    @property
    def act_disk_percent(self):
        return 100 - self.act_unified_percent
...
def init_weight_list(weight_specs, policy, env):
    dev_percents = [policy.w_disk_percent, policy.w_unified_percent]
    dev_choices = [env.disk, env.unified]
...
class InputEmbed:
    ...
    def init_weight(self, weight_home, path):
        ...
        weight_home.store(new_state_dict)

    def load_weight(self, weight_home, weight_read_buf, k):
        if k == 0:
            dst = self.weight_load_dst
            weight_read_buf.store(weight_home['embed_tokens.weight'].smart_copy(dst))
    ...
    def forward(self, hidden,  weight_read_buf, attention_mask,
                cache_write_buf, i, k):
...
class OutputEmbed:
    ...
    def load_weight(self, weight_home, weight_read_buf, k):
        ...
        if k == 0:
            ...
            dst2 = self.compute
            weight_read_buf.store((weight_home['norm.weight'].smart_copy(dst2),
                weight_home['lm_head.weight'].smart_copy(dst1)))
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if k == self.policy.num_gpu_batches - 1:
            # Clear the weight_read_buf if it is the last gpu batch
            w_norm, w_lm_head = weight_read_buf.pop()
            ####버퍼 flush
        else:
            w_norm, w_lm_head = weight_read_buf.val

        h = self.compute.output_embed(h, w_norm, w_lm_head, donate,
...
class SelfAttention:
    def __init__(self, config, env, policy):
        ...
        self.attention_compute = self.env.gpu
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if i == 0:  # prefill
            ...
            self, hidden, attention_mask, w_qkv,
            w_out, w_ln, n_head, n_kv_head, donate, start_pos, nl, nb, cache_read_buf, device, cache_write_buf
            h, new_k_cache, new_v_cache = self.compute.mha(h, mask, w_qkv,
                w_out, w_ln, n_head, donate, i, )
...
class MLP:
    ...
    def forward(self, hidden, weight_read_buf, i, k):
        donate = [False] * 7
        h, donate[0] = hidden.val, True

        if k == self.policy.num_gpu_batches - 1:
            # Clear the weight_read_buf if it is the last gpu batch
            w_up, w_down = weight_read_buf.pop()
        else:
            w_up, w_down = weight_read_buf.val

        h = self.compute.mlp(h, w_up, w_down)
        hidden.val = h


class Phi3DecoderLayer:
    def __init__(self, config, env, policy, layer_idx, i):
            ...
            with safe_open(file_path, framework="pt", device="cuda") as f:
                for name in partial_model_keys:
                    try:
                        ...
                        #partial_model_keys.remove(name)
                    except:
                        pass
            
        
        if k == 0:
            weight_read_buf.store(new_state_dict)
