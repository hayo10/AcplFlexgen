...
class TorchDevice:
    ...
    def opt_input_embed(self, inputs, w_token, w_pos, pad_token_id, donate):
        # decompress weights
    ...
    def opt_output_embed(self, inputs, w_ln, w_lmhead, donate,
        ...
        # output embedding
        logits = F.linear(hidden, w_lmhead.data)
    ...
    def mha(self, hidden, attention_mask, position_ids, w_qkv,
            w_out, w_ln, n_head, n_kv_head, donate, start_pos, nl, nb, cache_read_buf, device, cache_write_buf, comp_config):
        ...
        ##shape (b, s, n_head, head_dim)
        # keys = torch.concat(keys, xk, dim=1)
        # values = torch.concat(values, xv, dim=1)
        ...
        attn_output = flash_attn_with_kvcache(
            q = xq,
            k_cache = cache_read_buf[nl][nb][0],
            v_cache = cache_read_buf[nl][nb][1]
            k = xk,
            v = xv,
            rotary_cos = cos,
            rotary_sin = sin,
        ...
        attn_output = attn_output.reshape(b, q_len, self.hidden_size).contiguous()
